...
from services.linkedin_outreach.crypto import encrypt_pair
...
async def start_search(
    ...
    """Create a search job and immediately start an Apollo people search server-side."""
...
async def _run_apollo_search(
    ...
    """Search for people via Apollo.io and generate personalised messages."""
    from database.session import SessionLocal
    from services.lead_discovery.apollo_service import search_people as apollo_search
    ...
    try:
        ...
        # Build Apollo payload — no email filter needed for LinkedIn connect
        payload: dict = {
        ...
        # Apollo client is sync — run in thread so we don't block the event loop
        loop = asyncio.get_event_loop()
        result = await loop.run_in_executor(None, lambda: apollo_search(payload))

        raw_people = result.get("people", [])

        # Map Apollo format → our internal format
        people: list[dict] = []
        for p in raw_people:
        ...
        logger.info("Apollo search job %d completed: %d leads", job_id, len(people_with_messages))

    except Exception as e:
        logger.error("Apollo search job %d failed: %s", job_id, e, exc_info=True)
        job = db.query(LinkedInSearchJob).filter(LinkedInSearchJob.id == job_id).first()
        if job:
            job.status = "failed"
