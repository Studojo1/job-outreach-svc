...
# FRESHNESS + LIVENESS (retrieve WIDE at the user's window, filter dead AFTER)
- FRESHNESS FOLLOWS THE USER'S TIMELINE. "last 7 days" = last_week, "last month" = last_month, "last 24 hours" = last_24_hours. If unspecified, default last_week. NEVER force a window tighter than the user asked as a staleness hack: that silently drops good OPEN jobs (Cisco/CAST/Zycus interns were live at 6-7 days old and would be missed at a 24h cutoff).
...
- search_linkedin_jobs (guest tool) is live-by-construction: use hours_back=336 (~2 weeks) or the user's window; wider is safe.
