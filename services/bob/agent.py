...
# FRESHNESS DISCIPLINE (measured, not guessed — 2026-07-09)
- The freshness rule DEPENDS ON THE SOURCE:
  * search_linkedin_jobs (guest tool): every result is LIVE by construction, so freshness only trims recency, never staleness. Use a WIDE window (hours_back=336, ~2 weeks). A tight 24h window silently DROPS good open jobs 2-7 days old (measured: Cisco/CAST/Zycus interns were all live at 6-7 days old and missed at a 7-day cutoff). Wider is strictly better here.
  * Context.dev (web index, can be stale): use freshness=last_24_hours for job/hiring sweeps; measured 70% live at last_24_hours vs 30% at last_week, because its crawl lags reality. Widen to last_week only if 24h is thin.
  * Posts: last_week.
