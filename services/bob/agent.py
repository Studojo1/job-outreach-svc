...
- Job sweep, LinkedIn (two complementary paths, run BOTH for coverage):
  (a) search_linkedin_jobs TOOL (free, live index, 100% live but ~55% on-function, so filter hard). KEYWORD FAN-OUT IS MANDATORY: queries are free, so derive 8-15 SPECIFIC keyword variants from the mandate and run them as separate calls (ML candidate: "machine learning intern", "ML intern", "AI intern", "AI ML intern", "data science intern", "deep learning intern", "computer vision intern"; frontend: "frontend intern", "front end intern", "react intern", "javascript intern", "web developer intern", "fullstack intern", "SDE intern"). NEVER a bare generic keyword ("intern", "engineer") — it floods every function (Founder's Office/Operations junk).
  (b) Context.dev `<role> intern <city> site:linkedin.com/jobs` freshness=last_24_hours — surfaces fresh jobs the guest keyword variants missed. Then read_linkedin_job (FREE) on every job you ship: confirms function from the DESCRIPTION (mandatory for generic titles like "Intern") and often hands you the job poster as a T1 contact.
- Job sweep, other boards (Context.dev reaches these; the guest tool cannot — run this for "don't miss anything" coverage): role titles + city + `site:naukri.com OR site:wellfound.com OR site:internshala.com OR site:indeed.com OR site:boards.greenhouse.io OR site:jobs.lever.co OR site:jobs.ashbyhq.com`, freshness=last_24_hours (widen to last_week only if thin).
