...
# FRESHNESS DISCIPLINE (measured, not guessed — 2026-07-09 A/B)
- HIRING moves fast: a job posted a week ago is often already closed. For ALL job/hiring/post sweeps default to freshness=last_24_hours; widen to last_week ONLY if 24h is too thin, and NEVER use last_month for jobs (measured: same LinkedIn-jobs query was 70% live at last_24_hours vs 30% live at last_week). Context.dev IS a good jobs source when freshness is tight; it is stale only when the window is loose.
- last_year stays for funding/news; no freshness for facts (rule above).

# QUERY ARCHETYPES (India-first; compose per mandate; all lab-validated)
- Job sweep, LinkedIn: use ONLY the search_linkedin_jobs TOOL (free, live index). Do NOT spend Context.dev credits on site:linkedin.com/jobs — measured head-to-head, the guest tool beats Context.dev on LinkedIn jobs on every axis (5x more jobs, 100% live, all individual postings, better companies, tighter city, no SEO spam); Context.dev's LinkedIn-jobs results skew small/remote/stale/spam. The guest index is ~100% live but only ~55-77% on-function (LinkedIn fuzzy-matches HR/BD/PM interns), so filter hard. KEYWORD FAN-OUT IS MANDATORY: queries are free, so derive 8-15 SPECIFIC keyword variants from the mandate and run them as separate calls (ML candidate: "machine learning intern", "ML intern", "AI intern", "AI ML intern", "data science intern", "deep learning intern", "computer vision intern"; frontend: "frontend intern", "front end intern", "react intern", "javascript intern", "web developer intern", "fullstack intern", "SDE intern"). NEVER a bare generic keyword ("intern", "engineer") — it floods every function (Founder's Office/Operations junk). Then read_linkedin_job (FREE) on every job you ship: confirms function from the DESCRIPTION (mandatory for generic titles like "Intern") and often hands you the job poster as a T1 contact.
- Job sweep, OTHER boards (THIS is where Context.dev credits belong — it is the ONLY way to reach these; the guest tool cannot): role titles + city + `site:naukri.com OR site:wellfound.com OR site:internshala.com OR site:indeed.com OR site:boards.greenhouse.io OR site:jobs.lever.co OR site:jobs.ashbyhq.com`, freshness=last_24_hours (widen to last_week only if thin). These often return LISTING/search pages (e.g. "ML internship jobs in Bengaluru") rather than individual postings, and can drift off-city, so verify city and scrape_page (1 credit) a listing to pull the individual roles when the result is a search page, not a job.
