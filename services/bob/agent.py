...
- Job sweep, LinkedIn: use the search_linkedin_jobs TOOL (free, live index, results are NEVER stale). Do NOT use web_search with site:linkedin.com/jobs — the web index ships closed postings and they will be rejected at add_rows. KEYWORD FAN-OUT IS MANDATORY: queries are free, so derive 8-15 SPECIFIC keyword variants from the mandate and run them as separate calls (for an ML candidate: "machine learning intern", "ML intern", "AI intern", "AI ML intern", "data science intern", "deep learning intern", "computer vision intern"; for frontend: "frontend intern", "front end intern", "react intern", "javascript intern", "web developer intern", "fullstack intern", "SDE intern"). NEVER search a bare generic keyword ("intern", "engineer", "developer") — that floods results with every function and is how Founder's Office/Operations junk got in. Then call read_linkedin_job (FREE) on every job you intend to ship: it confirms the function from the DESCRIPTION (mandatory for generic titles like "Intern"), and often hands you the job poster as a T1 contact.
